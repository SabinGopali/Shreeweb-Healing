...
const itemClass = ({ isActive }) =>
  `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${
    isActive ? 'bg-stone-800 text-white shadow-sm' : 'text-stone-700 hover:bg-stone-200/80'
  }`;

const groupLabel = 'px-3 pt-5 pb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-stone-500';
...
export default function CmsSidebar() {
  ...
  return (
    <>
      <aside className="hidden w-64 shrink-0 flex-col border-r border-stone-200/90 bg-[#EDE8DF] lg:flex">
      ...
      {/* Mobile drawer */}
      <div className="flex items-center justify-between border-b border-stone-200/90 bg-[#EDE8DF] px-4 py-3 lg:hidden">
        <span className="font-serif text-lg text-stone-900">Navigation</span>
        <button
          type="button"
          className="rounded-lg border border-stone-300 bg-white px-3 py-1.5 text-sm font-medium text-stone-800 transition hover:bg-stone-50"
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
        >
          {open ? 'Close' : 'Menu'}
